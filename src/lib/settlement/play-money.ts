/* eslint-disable style/max-statements-per-line */
import type { SettlementAdapter, SettlementRequest, SettlementResult } from './types'

const BASE_URL = process.env.AMM_BASE_URL || 'http://localhost:8000/api/v1'

function serviceKey() {
  return process.env.PLAY_MONEY_SERVICE_API_KEY?.trim() || process.env.TELLWISE_SECRET?.trim() || ''
}

export class PlayMoneySettlementAdapter implements SettlementAdapter {
  readonly name = 'play_money'

  async settle(request: SettlementRequest): Promise<SettlementResult> {
    if (request.direction === 'withdrawal') {
      return { status: 'succeeded', externalReference: `play-money-withdrawal:${request.paymentIntentId}` }
    }
    if (request.direction !== 'deposit' && request.direction !== 'refund') { return { status: 'pending', externalReference: request.paymentIntentId } }
    const key = serviceKey()
    if (!key) { return { status: 'failed', failureCode: 'SERVICE_AUTH_MISSING', failureMessage: 'Settlement service authentication is not configured.' } }
    const commonHeaders = {
      'content-type': 'application/json',
      'x-play-money-api-key': key,
      'x-tellwise-secret': process.env.TELLWISE_SECRET?.trim() || key,
      'x-tellwise-user-id': request.userId,
    }
    const syncResponse = await fetch(`${BASE_URL}/users/sync`, {
      method: 'POST',
      cache: 'no-store',
      headers: { ...commonHeaders, 'idempotency-key': `settlement-user-sync:${request.userId}` },
      body: JSON.stringify({ id: request.userId, username: `user_${request.userId.slice(0, 12)}`, email: `${request.userId}@tellwise.local`, role: 'USER' }),
    })
    if (!syncResponse.ok) { return { status: 'failed', failureCode: 'USER_SYNC_FAILED', failureMessage: 'Could not prepare the settlement account.' } }
    const response = await fetch(`${BASE_URL}/internal/users/${encodeURIComponent(request.userId)}/settlement-credit`, {
      method: 'POST',
      cache: 'no-store',
      headers: {
        ...commonHeaders,
        'x-tellwise-internal-operation': 'settlement-credit',
        'idempotency-key': `settlement-credit:${request.paymentIntentId}`,
      },
      body: JSON.stringify({ amount: request.amount, paymentIntentId: request.paymentIntentId }),
    })
    const payload = await response.json().catch(() => null)
    if (!response.ok) { return { status: 'failed', failureCode: `PLAY_MONEY_${response.status}`, failureMessage: payload?.error || 'Settlement service rejected the credit.' } }
    return { status: 'succeeded', externalReference: request.paymentIntentId, ledgerTransactionId: payload?.data?.transactionId }
  }
}
